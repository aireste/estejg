...
export default function AnimatedGradient({
  ...
  return (
    <div
      ref={containerRef}
      className={className}
      style={{ position: "absolute", inset: 0, zIndex: 0, borderRadius: radius, overflow: "hidden", ...style }}
    >
...
vec4 blend_colors(vec4 c1,vec4 c2,vec4 c3,float mixer,float edgesWidth,float edge_blur) {
  vec3 col1=c1.rgb*c1.a; vec3 col2=c2.rgb*c2.a; vec3 col3=c3.rgb*c3.a;
  float r1=smoothstep(.0+.35*edgesWidth,.7-.35*edgesWidth+.5*edge_blur,mixer);
  float r2=smoothstep(.3+.35*edgesWidth,1.-.35*edgesWidth+edge_blur,mixer);
  vec3 bc=mix(col1,col2,r1); float bo=mix(c1.a,c2.a,r1);
  vec3 cc=mix(bc,col3,r2); float co=mix(bo,c3.a,r2);
  return vec4(cc,co);
}
void main() {
  ...
  if(u_shape<.5) {
    ...
    float shape=smoothstep(.45-ss,.55+ss,sh+.3*(proportion-.5));
    mixer=shape;
  }
